...
public class TaxonCell extends MenuExtendedCell<Taxon> {
	...
	public TaxonCell(EventBus eventBus, TaxonMatrix taxonMatrix) {
	...
	public void render(Context context, Taxon value, SafeHtmlBuilder sb) {
		...
		String quickTipText = value.getFullName();
		quickTipText += "<br>" + "Coverage: " + value.getTaxonMatrix().getCoverage(value);
	...
	protected Menu createContextMenu(final int colIndex, final int rowIndex) {
		final Menu menu = new TaxonMenu(eventBus, taxonMatrix, taxonMatrix.getTaxon(rowIndex));
		return menu;
	}
}
